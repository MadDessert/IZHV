...
public class Gun : MonoBehaviour
{
     ...
     public void ShootGun(Transform director)
    {
        if (shotgun)
        {
            ...
            float angleStep = shotgunBullets > 1 ? shotgunSpread / (float)(shotgunBullets - 1) : 0.0f;

            for (int i = 0; i < shotgunBullets; i++)
            {
                // Boring math :/
                float currentAngleOffset = startAngle + (angleStep * i);
                Quaternion rotationOffset = Quaternion.Euler(0.0f, 0.0f, currentAngleOffset);
                Quaternion finalRotation = director.rotation * rotationOffset;
                Vector3 bulletPosition = director.position + (finalRotation * Vector3.up * spawnOffset);

                SpawnBullet(bulletPosition, finalRotation);
            }
        }
        ...
        else
        {
            ...
            Vector3 bulletPosition = director.position + (director.rotation * Vector3.up * spawnOffset);
