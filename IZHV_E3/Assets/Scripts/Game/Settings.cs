...
public class Settings : MonoBehaviour
{
    ...
    {
        // 1. Register the new player GameObject 
        int newIndex = AddPlayer(playerInput.gameObject);

        // 2. Configure the Player Script
        var playerScript = playerInput.GetComponent<Player>();
        if (playerScript != null)
        {
            playerScript.primaryPlayer = (newIndex == 0);
        }

        Debug.Log($"Player joined! Assigned Index: {newIndex}");
    }

    public void OnPlayerLeft(PlayerInput playerInput)
    {
        // 1. Remove the player from our list
